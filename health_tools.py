...
每个工具都只依赖 HealthStore，不依赖 AstrBot 内部 API（除了
FunctionTool 基类），便于复用与测试。
"""
...
class HealthLatestTool(FunctionTool[AstrAgentContext]):
    ...
    async def call(self, context: ContextWrapper[AstrAgentContext], **kwargs) -> str:
        ...
        if metric == "heart_rate":
            latest = self.store.latest_hr(minutes=30)
            if latest is None:
        ...
        # overview
        latest = self.store.latest_hr(minutes=60)
        batteries = self.store.battery()
        last_data = self.store.last_data_time()
        parts = []
        if latest:
...
class HealthStepsTool(FunctionTool[AstrAgentContext]):
    ...
    description: str = (
        "查询某一天的步数。date 为 YYYY-MM-DD，或 today / yesterday。"
        "不传时默认今天。"
    )
    parameters: dict = Field(
        default_factory=lambda: {
...
class HealthSleepTool(FunctionTool[AstrAgentContext]):
    ...
    description: str = (
        ...
        "date 为 YYYY-MM-DD，或 today / yesterday；不传时默认今天（即“昨晚”）。"
    )
    parameters: dict = Field(
        default_factory=lambda: {
            "type": "object",
    ...
    async def call(self, context: ContextWrapper[AstrAgentContext], **kwargs) -> str:
        ...
        summary = self.store.sleep_summary(day)
...
class HealthAlertsTool(FunctionTool[AstrAgentContext]):
    ...
    description: str = "查询最近几天的健康告警（心率过高、电量过低）。days 默认 3。"
    parameters: dict = Field(
        default_factory=lambda: {
            "type": "object",
            "properties": {
    ...
    async def call(self, context: ContextWrapper[AstrAgentContext], **kwargs) -> str:
        try:
            ...
            days = 3
        alerts = self.store.recent_alerts(days)
        if not alerts:
