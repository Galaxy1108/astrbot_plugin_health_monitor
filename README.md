...
- **LLM 工具**（自然语言直接问，仅返回当前会话已绑定设备的数据）：
  ...
  - `health_steps` → “今天走了多少步”（`detail=true` 返回完整分钟步数曲线）
  - `health_sleep` → “昨晚睡眠怎么样”（前一晚 20:00 → 当天 12:00，含深睡/浅睡/REM/清醒分钟数；`detail=true` 返回逐分钟睡眠分期时间线）
  - `health_hr_history` → “今天心率怎么样”“完整心率曲线”（`detail=true` 返回逐分钟心率曲线）
  - `health_alerts` → “最近有没有告警”
  - `health_extended` → 扩展指标：血氧 SpO2 / 压力 / HRV(RR 间期) / 睡眠呼吸率 / 睡眠时段 / 每日汇总 / PAI / 运动记录（含类型/距离/卡路里/平均心率）/ 运动过程心率（`detail=true` 返回完整逐点数据）
  - 所有逐点数据超过 200 条时自动写入临时文件，AI 读取后即删，不占上下文
